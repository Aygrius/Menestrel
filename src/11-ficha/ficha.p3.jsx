...
function FichaPersonagem({ ac, lang, currentUserId, pjAtivoId, onVoltar, onTrocar, onEditar, onExcluir, isMestre }) {
  ...
  /* ============================== [11] SELO DE BRONZE — equipamento radial ==============================
     Retrato grande ao centro, 2 anéis concêntricos de medalhões ao redor.
     Reaproveita fpSlotTile/ocupanteDefesa/ocupanteDedo tal como estão —
     muda só o posicionamento (radial via CSS var --fp3-angle), não o
     comportamento de clique/tooltip/desequipar.

     ATENÇÃO — CONVENÇÃO DE ÂNGULO DO --fp3-angle (já corrigida aqui,
     não recalcular de novo do zero sem reler isto): o CSS aplica
     transform: rotate(angle) translate(r) rotate(-angle). Como
     translate(r) desloca puramente no eixo X (para a DIREITA) antes
     da rotação, --fp3-angle: 0deg cai na posição das 3 horas
     (direita), NÃO no topo. Os ângulos abaixo já estão deslocados
     -90° em relação à intuição "0°=topo" para compensar isso — uma
     tentativa anterior usou ângulos sem essa correção e todo o selo
     saiu rotacionado 90° em relação ao desenho de referência do
     usuário. Fórmula pra novos ajustes: ângulo_css = ângulo_desejado
     (0°=topo, sentido horário) − 90°.

     14 posições visuais, calibradas a partir de um desenho de
     referência do usuário (mapa com posições cardinais/diagonais):

     Anel INTERNO (hexágono, 60° entre vizinhos):
       topo-esq Capacete · topo-dir Peito
       esq Ombros · dir Luvas (braços)
       base-esq Pernas · base-dir Pés

     Anel EXTERNO (octógono, 45° entre vizinhos):
       topo Capa
       topo-esq Colar (pescoço) · topo-dir Brinco (orelha)
       esq Espada (mão_e) · dir Escudo (mão_d)
       base-esq Anel (dedo) · base Cinto · base-dir Anel (dedo) */
  const FP3_INNER = [
    ['cabeca',  () => ocupanteDefesa('cabeca', 0), -120],
    ['peito',   () => ocupanteDefesa('peito', 0),   -60],
    ['bracos',  () => ocupanteDefesa('bracos', 0),    0],
    ['pes',     () => ocupanteDefesa('pes', 0),      60],
    ['pernas',  () => ocupanteDefesa('pernas', 0),  120],
    ['ombros',  () => ocupanteDefesa('ombros', 0),  180],
  ];
  const FP3_OUTER = [
    ['capa',    () => ocupanteDefesa('capa', 0),    -90],
    ['orelha',  () => ocupanteDefesa('orelha', 0),  -45],
    ['mao_d',   () => ocupanteDefesa('mao_d', 0),     0],
    ['dedos',   () => ocupanteDedo(0),               45],
    ['cintura', () => ocupanteDefesa('cintura', 0),  90],
    ['dedos',   () => ocupanteDedo(1),              135],
    ['mao_e',   () => ocupanteDefesa('mao_e', 0),   180],
    ['pescoco', () => ocupanteDefesa('pescoco', 0), -135],
  ];
  // Posiciona cada medalhão no ângulo --fp3-angle (graus, já definido
  // por slot acima, calibrado pelo desenho de referência), lido pelo
  // CSS com transform: rotate(var(--fp3-angle)) translate(var(--fp3-r))
  // rotate(calc(var(--fp3-angle) * -1)) — a dupla rotação mantém o
  // ícone "de pé" (não roda com a posição).
  const fp3Ring = (defs, keyPrefix) => {
    const seen = {};
    return defs.map(([regiao, getOcupante, angle], i) => {
      const occCount = (seen[regiao] = (seen[regiao] ?? -1) + 1);
      return (
        <div
          key={keyPrefix + '-' + regiao + '-' + occCount}
          className="fp3-medal"
          style={{ '--fp3-angle': angle + 'deg' }}
        >
          {fpSlotTile(regiao, getOcupante(), keyPrefix + '-tile-' + i)}
        </div>
      );
    });
  };
  const fp3Selo = (
    <div className="fp3-selo">
      <div className="fp3-ring fp3-ring--outer">{fp3Ring(FP3_OUTER, 'outer')}</div>
      <div className="fp3-ring fp3-ring--inner">{fp3Ring(FP3_INNER, 'inner')}</div>
      <div
        className={'fp3-portrait' + (podeEditarFoto ? ' is-editable' : '') + (!fotoUrl ? ' is-empty' : '')}
      ...
      >
        {fotoUrl
          ? <img src={fotoUrl} alt={nomeCompleto} className="fp3-portrait-img" />
          : <span className="fp3-mono">{inicial}</span>}
        {podeEditarFoto && (
          <span className="fp3-portrait-cam">
  ...
  // Mini-grade de Combate (4 colunas) — Defesa · Velocidade · Res. Física · Res. Mágica.
  const fp3Stat = (label, value, icon, accent) => (
    <div className="fp3-stat" key={label}>
      <i className={'ti ' + icon} aria-hidden="true" />
      <span className={'fp3-stat-v' + (accent ? ' is-gold' : '')}>{value}</span>
      <span className="fp3-stat-l">{label}</span>
    </div>
  );
  const fp3Combat = (
    <div className="fp3-combat-grid">
      {fp3Stat(en ? 'Defense' : 'Defesa', _d.defesa ?? '—', 'ti-shield-chevron', true)}
      {fp3Stat(en ? 'Speed' : 'Velocidade', velocidade ?? '—', 'ti-bolt')}
      {fp3Stat(en ? 'Phys. Res.' : 'Res. Física', _d.rf ?? _d.resistenciaFisica ?? '—', 'ti-sword')}
      {fp3Stat(en ? 'Magic Res.' : 'Res. Mágica', _d.rm ?? _d.resistenciaMagica ?? '—', 'ti-meteor')}
    </div>
  );

  // Condições como bolinhas — clique abre o MESMO popover de edição
  // (abrirEdicaoBarra/BarEditPopover) já usado pelas barras grandes.
  // Sem onEdit (jogador comum), a bolinha fica só leitura (title no hover).
  const fp3Cond = (
    <div className="fp3-cond-pills">
      {condBars.map((c) => {
        const pct = c.max > 0 ? Math.max(0, Math.min(1, c.val / c.max)) : 0;
        const cor = fichaCondColor(pct);
        const abrir = podeEditarEstado
          ? (e) => abrirEdicaoBarra(c, 'cond', e.currentTarget.getBoundingClientRect())
          : undefined;
        return (
          <button
            key={c.key}
            type="button"
            className={'fp3-pill' + (podeEditarEstado ? ' is-editable' : '')}
            onClick={abrir}
            title={`${c.label}: ${c.val}/${c.max}`}
          >
            <span className="fp3-pill-dot" style={{ background: cor }} />
            <span className="fp3-pill-lbl">{c.label}</span>
          </button>
        );
      })}
    </div>
  ...
  // Atributos-base (mantidos como cartão próprio, igual ao painel
  // fp2-panel original).
  const ATTR_DEFS = [
    ...
    <div className={'menestrel-ui fp-page' + (fpFull ? ' is-full' : '') + ((fpTab === 'inventario' || fpTab === 'loja' || fpTab === 'diario') ? ' is-inv' : '')}>
      ...
      <div className="fp2-sheet">
        <div className="fp2-frame">
          ...
          {fotoErro && <div className="fp3-erro">{fotoErro}</div>}
          {eqErro && <div className="fp3-erro">{eqErro}</div>}

          <div className="fp3-col">
            {/* ── Layout em 3 colunas (pedido do usuário, repensando
                o header anterior de 2 colunas selo+side):
                ESQUERDA  (.fp3-left)  — nome/título, vitalidade,
                           Condições, mini-grade de combate.
                CENTRO    (.fp3-center) — só o Selo de Bronze (retrato
                           + 2 anéis de medalhões de equipamento).
                DIREITA   (.fp3-right) — Arsenal, Atributos.
                Esquerda e direita são simétricas (mesma largura,
                max-width igual via CSS); o selo central encolheu de
                560px pra ~480px pra abrir espaço real pras duas
                colunas — calibrado em index.css, ver .fp3-selo.
                Abaixo de 1100px (tela estreita), tudo colapsa de
                volta pro empilhamento vertical ANTIGO — selo
                primeiro, depois esquerda, depois direita — não dá
                pra espremer 3 colunas numa tela sem espaço real;
                ver media query em index.css (.fp3-header). */}
            <div className="fp3-header">
              <div className="fp3-left">
                <div className="fp3-name-block">
                  <h2 className="fp3-name">{nomeCompleto}</h2>
                  {(titulo || metaLinha) && (
                    <div className="fp3-meta">{[titulo, metaLinha].filter(Boolean).join(' · ')}</div>
                  )}
                </div>
                <div className="fp3-vit-band">{elVit}</div>
                {/* Condições — card próprio com filete de gradiente
                    no topo (.fp3-card--cond, mesmo padrão visual de
                    .inv-card/.pj-card/.hist-card) + border-radius:6px
                    (pedidos do usuário em rodada anterior). */}
                <section className="fp3-card fp3-card--inline fp3-card--cond">
                  <header className="fp3-card-head"><i className="ti ti-activity-heartbeat" aria-hidden="true" /><h3>{en ? 'Conditions' : 'Condições'}</h3></header>
                  <div className="fp3-card-body">{fp3Cond}</div>
                </section>
                {/* Mini-grade de combate: embaixo de Condições, ainda
                    na coluna esquerda (pedido anterior do usuário). */}
                {fp3Combat}
              </div>

              <div className="fp3-center">
                {fp3Selo}
              </div>

              <div className="fp3-right">
                {/* Arsenal: agora na coluna DIREITA, acima de
                    Atributos (pedido do usuário). A tabela (.fp-atk,
                    min-width 480px) pode ficar mais larga que a
                    coluna; .fp3-arsenal mantém overflow-x:auto (ver
                    index.css) pra rolar em vez de quebrar o layout
                    quando isso acontecer. */}
                {temArmas && (
                  <section className="fp3-card fp3-card--inline">
                    <header className="fp3-card-head"><i className="ti ti-swords" aria-hidden="true" /><h3>{en ? 'Arsenal' : 'Arsenal'}</h3></header>
                    <div className="fp3-card-body fp3-arsenal">{elArmas}</div>
                  </section>
                )}
                <section className="fp3-card fp3-card--inline">
                  <header className="fp3-card-head"><i className="ti ti-star" aria-hidden="true" /><h3>{en ? 'Attributes' : 'Atributos'}</h3></header>
                  <div className="fp3-card-body fp3-attrs-grid">
                ...
                <div className="det-act-row">
                  {isCont && (
                    <button className="det-act det-act-primary"
                      ...
                      <span className="det-act-lbl">{en ? 'View contents' : 'Ver conteúdo'}</span>
                    </button>
                  )}
